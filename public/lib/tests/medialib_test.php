...
final class medialib_test extends \advanced_testcase {
    ...
    public function test_get_filename(): void {
        ...
        $this->assertSame('frog.mp4', $manager->get_filename(new url(
    ...
    public function test_get_extension(): void {
        ...
        $this->assertSame('mp4', $manager->get_extension(new url(
                '/pluginfile.php/312/mod_page/content/7/frog.mp4')));
        $this->assertSame('', $manager->get_extension(new url(
    ...
    public function test_list_supported_urls(): void {
        ...
        // Some example URLs.
        $supported1 = new url('http://example.org/1.test');
        $supported2 = new url('http://example.org/2.TST');
        $unsupported = new url('http://example.org/2.jpg');
    ...
    public function test_can_embed_url(): void {
        ...
        $url = new url('http://example.org/test.mp4');
        $manager = core_media_manager::instance();
    ...
    public function test_embed_url_fallbacks(): void {
        ...
        foreach ($mediaformats as $format) {
            $url = new url('http://example.org/test.' . $format);
    ...
    public function test_embed_url_swf(): void {
        ...
        // Without any options...
        $url = new url('http://example.org/test.swf');
        $t = $manager->embed_url($url);
    ...
    public function test_slash_arguments(): void {
        ...
        // Format: mp3.
        $url = new url('http://example.org/pluginfile.php?file=x/y/z/test.mp3');
        $t = $manager->embed_url($url);
    ...
    public function test_embed_or_blank(): void {
        ...
        $url = new url('http://example.org/test.ogg');
    ...
    public function test_embed_url_size(): void {
        ...
        $url = new url('http://example.org/test.mp4?d=123x456');
        $t = $manager->embed_url($url, '', '666', '101');
    ...
    public function test_embed_alternatives(): void {
        ...
        $urls = array(
            new url('http://example.org/test.mp4'),
            new url('http://example.org/test.ogv'),
            new url('http://example.org/test.webm'),
            new url('http://example.org/test.flv'),
        );
