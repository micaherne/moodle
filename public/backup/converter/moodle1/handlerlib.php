...
require_once(dirname(__DIR__, 2) . '/util/xml/xml_writer.class.php');
require_once(dirname(__DIR__, 2) . '/util/xml/output/xml_output.class.php');
require_once(dirname(__DIR__, 2) . '/util/xml/output/file_xml_output.class.php');
...
class moodle1_course_outline_handler extends moodle1_xml_handler {
    ...
    public function process_course_module($data, $raw) {
        ...
        // host...
        $moddir = \core_component::get_plugin_directory('mod', $data['modulename']);
        if ($moddir && file_exists("$moddir/version.php")) {
            $plugin = new stdClass();
            $plugin->version = null;
            $module = $plugin;
