...
if ($ADMIN->fulltree) {
    $settings->add(new configtext('block_rss_client_num_entries', get_string('numentries', 'block_rss_client'),
                       get_string('clientnumentries', 'block_rss_client'), 5, PARAM_INT));

    $settings->add(new configtext('block_rss_client_timeout', get_string('timeout2', 'block_rss_client'),
    ...
    $settings->add(new heading('block_rss_addheading', '', $link));
}