...
if ($ADMIN->fulltree) {
    ...
    if (!function_exists('ldap_connect')) {
        ...
        $settings->add(new heading('auth_ldap_noextension', '', $OUTPUT->render($notify)));
    } else {
        ...
        $settings->add(new heading('auth_ldap/pluginname', '',
                new lang_string('auth_ldapdescription', 'auth_ldap')));

        // LDAP server settings.
        $settings->add(new heading('auth_ldap/ldapserversettings',
                new lang_string('auth_ldap_server_settings', 'auth_ldap'), ''));

        // Host.
        $settings->add(new configtext('auth_ldap/host_url',
        ...
        $versions[2] = '2';
        $versions[3] = '3';
        $settings->add(new configselect('auth_ldap/ldap_version',
        ...
        $settings->add(new configselect('auth_ldap/start_tls',
                new lang_string('start_tls_key', 'auth_ldap'),
                new lang_string('start_tls', 'auth_ldap'), 0 , $yesno));


        // Encoding.
        $settings->add(new configtext('auth_ldap/ldapencoding',
        ...
        // Search subcontexts.
        $settings->add(new configselect('auth_ldap/search_sub',
        ...
        $settings->add(new configselect('auth_ldap/opt_deref',
        ...
        $settings->add(new heading('auth_ldap/ldapforcepasswordchange',
                new lang_string('forcechangepassword', 'auth'), ''));

        // Force Password change.
        $settings->add(new configselect('auth_ldap/forcechangepassword',
                new lang_string('forcechangepassword', 'auth'),
                new lang_string('forcechangepasswordfirst_help', 'auth'), 0 , $yesno));

        // Standard Password Change.
        $settings->add(new configselect('auth_ldap/stdchangepassword',
        ...
        $settings->add(new heading('auth_ldap/passwordexpire',
        ...
        // Add the setting.
        $settings->add(new configselect('auth_ldap/expiration',
                new lang_string('auth_ldap_expiration_key', 'auth_ldap'),
                $description, 0 , $expiration));

        // Password Expiration warning.
        $settings->add(new configtext('auth_ldap/expiration_warning',
        ...
        // Grace Logins.
        $settings->add(new configselect('auth_ldap/gracelogins',
        ...
        // User Creation.
        $settings->add(new heading('auth_ldap/usercreation',
                new lang_string('auth_user_create', 'auth'), ''));

        // Create users externally.
        $settings->add(new configselect('auth_ldap/auth_user_create',
        ...
        $settings->add(new configtext('auth_ldap/create_context',
        ...
        $settings->add(new heading('auth_ldap/systemrolemapping',
        ...
        foreach ($roles as $role) {
            ...
            if (core_text::strlen($role['settingname']) > 100 || !preg_match('/^[a-zA-Z0-9_]+$/', $role['settingname'])) {
                $url = new url('/admin/roles/define.php', array('action' => 'edit', 'roleid' => $role['id']));
                $a = (object)['rolename' => $role['localname'], 'shortname' => $role['shortname'], 'charlimit' => 93,
                    'link' => $url->out()];
                $settings->add(new heading('auth_ldap/role_not_mapped_' . sha1($role['settingname']), '',
                    get_string('cannotmaprole', 'auth_ldap', $a)));
            } else {
                $settings->add(new configtext('auth_ldap/' . $role['settingname'],
        ...
        // User Account Sync.
        $settings->add(new heading('auth_ldap/syncusers',
        ...
        $settings->add(new configselect('auth_ldap/ntlmsso_ie_fastpath',
        ...
        $settings->add(new configselect('auth_ldap/ntlmsso_type',
    ...
    $authplugin = \core\di::get(\core\authentication::class)->get_plugin('ldap');
