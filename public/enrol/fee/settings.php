...
if ($ADMIN->fulltree) {
    ...
    if (empty($currencies)) {
        ...
        $settings->add(new heading('enrol_fee_nocurrency', '', $OUTPUT->render($notify)));
    }

    $settings->add(new heading('enrol_fee_settings', '', get_string('pluginname_desc', 'enrol_fee')));
    ...
    $settings->add(new heading('enrol_fee_defaults',
        get_string('enrolinstancedefaults', 'admin'), get_string('enrolinstancedefaults_desc', 'admin')));

    $options = array(ENROL_INSTANCE_ENABLED  => get_string('yes'),
                     ENROL_INSTANCE_DISABLED => get_string('no'));
    $settings->add(new configselect('enrol_fee/status',
        get_string('status', 'enrol_fee'), get_string('status_desc', 'enrol_fee'), ENROL_INSTANCE_DISABLED, $options));

    if (!empty($currencies)) {
        $settings->add(new configtext('enrol_fee/cost', get_string('cost', 'enrol_fee'), '', 0, PARAM_FLOAT, 4));
        $settings->add(new configselect('enrol_fee/currency', get_string('currency', 'enrol_fee'), '', 'USD',
            $currencies));
    }
    ...
    if (!during_initial_install()) {
        ...
        $settings->add(new configselect('enrol_fee/roleid',
            get_string('defaultrole', 'enrol_fee'), get_string('defaultrole_desc', 'enrol_fee'), $student->id ?? null, $options));
    }

    $settings->add(new configduration('enrol_fee/enrolperiod',
