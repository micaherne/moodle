...
$pageurl = new url('/enrol/lti/index.php', array('courseid' => $courseid, 'legacy' => $legacy));
...
if ($action) {
    if ($action === 'delete') {
        if ($ltiplugin->can_delete_instance($instance)) {
            ...
            $yesurl = new url('/enrol/lti/index.php',
...
$tabs = [
    0 => [
        new tabobject('0', new url('/enrol/lti/index.php', ['courseid' => $courseid]),
            get_string('lti13', 'enrol_lti')),
        new tabobject('1', new url('/enrol/lti/index.php', ['legacy' => 1, 'courseid' => $courseid]),
...
if ($ltiplugin->can_add_instance($course->id)) {
    echo $OUTPUT->single_button(new url('/enrol/editinstance.php',
        array(
            'legacy' => $legacy,
            'type' => 'lti',
            'courseid' => $course->id,
            'returnurl' => new url('/enrol/lti/index.php', ['courseid' => $course->id, 'legacy' => $legacy]))
        ),
        get_string('add'));
}
