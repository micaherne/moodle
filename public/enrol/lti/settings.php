...
$ADMIN->add('enrolments', new category('enrolltifolder', new lang_string('pluginname', 'enrol_lti'),
    $this->is_enabled() === false));

$settings = new settingpage($section, "User default values", 'moodle/site:config', $this->is_enabled() === false);
// Add all the user default values settings to the first page.
if ($ADMIN->fulltree) {
    $settings->add(new heading('enrol_lti_settings', '', get_string('pluginname_desc', 'enrol_lti')));

    if (!\core\di::get(\core\authentication::class)->is_enabled('lti')) {
        $notify = new \core\output\notification(get_string('authltimustbeenabled', 'enrol_lti'),
            \core\output\notification::NOTIFY_WARNING);
        $settings->add(new heading('enrol_lti_enable_auth_lti', '', $OUTPUT->render($notify)));
    }

    if (empty($CFG->allowframembedding)) {
        $notify = new \core\output\notification(get_string('allowframeembedding', 'enrol_lti'),
            \core\output\notification::NOTIFY_WARNING);
        $settings->add(new heading('enrol_lti_enable_embedding', '', $OUTPUT->render($notify)));
    }

    $settings->add(new heading('enrol_lti_user_default_values',
    ...
    $settings->add(new configselect('enrol_lti/emaildisplay', get_string('emaildisplay'),
    ...
    $settings->add(new configtext('enrol_lti/city', get_string('city'), '', $city));
    ...
    $settings->add(new configselect('enrol_lti/country', get_string('selectacountry'), '', $country,
        $countries));

    $settings->add(new configselect('enrol_lti/timezone', get_string('timezone'), '', 99,
        core_date::get_list_of_timezones(null, true)));

    $settings->add(new configselect('enrol_lti/lang', get_string('preferredlanguage'), '', $CFG->lang,
        get_string_manager()->get_list_of_translations()));

    $settings->add(new configtext('enrol_lti/institution', get_string('institution'), '', ''));
}
$ADMIN->add('enrolltifolder', $settings);

// Now, create a tool registrations settings page.
$settings = new settingpage('enrolsettingslti_registrations', "Tool registration", 'moodle/site:config',
    $this->is_enabled() === false);

$settings->add(new heading('enrol_lti_tool_registrations_heading',
...
$ADMIN->add('enrolltifolder', new externalpage('enrolsettingslti_registrations_edit',
...
$ADMIN->add('enrolltifolder', new externalpage('enrolsettingslti_deployment_manage',
