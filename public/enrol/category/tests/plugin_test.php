...
final class plugin_test extends \advanced_testcase {
    ...
    public function test_handler_sync(): void {
        ...
        role_assign($managerrole->id, $user1->id, coursecat::instance($cat1->id));
        role_assign($managerrole->id, $user3->id, course::instance($course1->id));
        role_assign($managerrole->id, $user3->id, course::instance($course2->id));
        $this->assertEquals(0, $DB->count_records('user_enrolments', array()));

        role_assign($studentrole->id, $user1->id, coursecat::instance($cat2->id));
        $this->assertTrue(is_enrolled(course::instance($course2->id), $user1->id));
        $this->assertTrue(is_enrolled(course::instance($course3->id), $user1->id));
        $this->assertTrue(is_enrolled(course::instance($course4->id), $user1->id));
        $this->assertEquals(3, $DB->count_records('user_enrolments', array()));

        role_assign($managerrole->id, $user2->id, coursecat::instance($cat3->id));
        $this->assertEquals(3, $DB->count_records('user_enrolments', array()));

        role_assign($teacherrole->id, $user4->id, coursecat::instance($cat1->id));
        $this->assertTrue(is_enrolled(course::instance($course1->id), $user4->id));
        $this->assertEquals(4, $DB->count_records('user_enrolments', array()));

        // Test role unassigned event.

        role_unassign($teacherrole->id, $user4->id, coursecat::instance($cat1->id)->id);
        $this->assertFalse(is_enrolled(course::instance($course1->id), $user4->id));
        $this->assertEquals(3, $DB->count_records('user_enrolments', array()));

        // Make sure handlers are disabled when plugin disabled.

        $this->disable_plugin();
        role_unassign($studentrole->id, $user1->id, coursecat::instance($cat2->id)->id);
        $this->assertEquals(3, $DB->count_records('user_enrolments', array()));

        role_assign($studentrole->id, $user3->id, coursecat::instance($cat1->id));
        $this->assertEquals(3, $DB->count_records('user_enrolments', array()));
    ...
    public function test_sync_course(): void {
        ...
        $this->assertTrue(is_enrolled(course::instance($course2->id), $user1->id));
        $this->assertFalse(is_enrolled(course::instance($course3->id), $user1->id));
        $this->assertFalse(is_enrolled(course::instance($course4->id), $user1->id));
        ...
        $this->assertFalse(is_enrolled(course::instance($course1->id), $user1->id));
        $this->assertTrue(is_enrolled(course::instance($course2->id), $user1->id));
        $this->assertTrue(is_enrolled(course::instance($course3->id), $user1->id));
        $this->assertTrue(is_enrolled(course::instance($course4->id), $user1->id));
        $this->assertEquals(3, $DB->count_records('user_enrolments', array()));
        ...
        $this->disable_plugin(); // Stops the event handlers.
        role_assign($studentrole->id, $user2->id, coursecat::instance($cat1->id));
        role_assign($teacherrole->id, $user4->id, coursecat::instance($cat1->id));
        role_unassign($studentrole->id, $user1->id, coursecat::instance($cat2->id)->id);
        $this->assertEquals(3, $DB->count_records('user_enrolments', array()));
        $this->enable_plugin();
        enrol_category_sync_course($course2);
        $this->assertFalse(is_enrolled(course::instance($course2->id), $user1->id));
        $this->assertFalse(is_enrolled(course::instance($course2->id), $user2->id));
        $this->assertFalse(is_enrolled(course::instance($course2->id), $user4->id));
        enrol_category_sync_course($course1);
        enrol_category_sync_course($course3);
        enrol_category_sync_course($course4);
        $this->assertEquals(2, $DB->count_records('user_enrolments', array()));
        $this->assertTrue(is_enrolled(course::instance($course1->id), $user2->id));
        $this->assertTrue(is_enrolled(course::instance($course1->id), $user4->id));
    ...
    public function test_sync_full(): void {
        ...
        role_assign($studentrole->id, $user2->id, coursecat::instance($cat1->id));
        role_assign($teacherrole->id, $user4->id, coursecat::instance($cat1->id));
        role_assign($teacherrole->id, $user3->id, coursecat::instance($cat2->id));
        role_assign($managerrole->id, $user3->id, course::instance($course3->id));
        $this->enable_plugin();
        $result = enrol_category_sync_full($trace);
        $this->assertSame(0, $result);
        ...
        $this->assertTrue(is_enrolled(course::instance($course1->id), $user2->id));
        $this->assertTrue(is_enrolled(course::instance($course1->id), $user4->id));
        $this->assertTrue(is_enrolled(course::instance($course2->id), $user3->id));
        $this->assertTrue(is_enrolled(course::instance($course3->id), $user3->id));
        $this->assertTrue(is_enrolled(course::instance($course4->id), $user3->id));
