...
use core_admin\setting\setting\h5plib_handler_select;
...
// H5P overview.
$ADMIN->add('h5p', new externalpage('h5poverview', get_string('h5poverview', 'core_h5p'),
    new url('/h5p/overview.php'), ['moodle/site:config']));

// Manage H5P libraries page.
$ADMIN->add('h5p', new externalpage('h5pmanagelibraries', get_string('h5pmanage', 'core_h5p'),
    new url('/h5p/libraries.php'), ['moodle/site:config', 'moodle/h5p:updatelibraries']));
...
if (!empty($defaulth5plib)) {
    ...
    $settings = new settingpage('h5psettings', new lang_string('h5psettings', 'core_h5p'));
    $ADMIN->add('h5p', $settings);

    $settings->add(new h5plib_handler_select('h5plibraryhandler', new lang_string('h5plibraryhandler', 'core_h5p'),
        new lang_string('h5plibraryhandler_help', 'core_h5p'), $defaulth5plib));

    $setting = new configtextarea(
