...
if ($ADMIN->fulltree) {
    ...
    $settings->add(new configselect('workshop/grade', get_string('submissiongrade', 'workshop'),
                        get_string('configgrade', 'workshop'), 80, $grades));

    $settings->add(new configselect('workshop/gradinggrade', get_string('gradinggrade', 'workshop'),
    ...
    $settings->add(new configselect('workshop/gradedecimals', get_string('gradedecimals', 'workshop'),
    ...
    if (isset($CFG->maxbytes)) {
        ...
        $settings->add(new configselect('workshop/maxbytes', get_string('maxbytes', 'workshop'),
                            get_string('configmaxbytes', 'workshop'), 0, $options));
    }

    $settings->add(new configselect('workshop/strategy', get_string('strategy', 'workshop'),
    ...
    $settings->add(new configselect('workshop/examplesmode', get_string('examplesmode', 'workshop'),
    ...
    foreach ($allocators as $allocator => $path) {
        if (file_exists($settingsfile = $path . '/settings.php')) {
            $settings->add(new heading('workshopallocationsetting'.$allocator,
    ...
    foreach ($strategies as $strategy => $path) {
        if (file_exists($settingsfile = $path . '/settings.php')) {
            $settings->add(new heading('workshopformsetting'.$strategy,
    ...
    foreach ($evaluations as $evaluation => $path) {
        if (file_exists($settingsfile = $path . '/settings.php')) {
            $settings->add(new heading('workshopevalsetting'.$evaluation,
